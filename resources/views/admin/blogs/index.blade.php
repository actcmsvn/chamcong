<x-app-layout title="{{ __('admin/menus.Blog') }}">
    <div class="container grid px-6 mx-auto">
        <header class="my-4 flex items-center justify-between mb-4">
            <h2 class="text-lg leading-6 font-semibold text-gray-700 dark:text-gray-200">{{ __('admin/menus.Blog') }}</h2>
            <a class="hover:bg-light-blue-200 hover:text-light-blue-800 group flex items-center rounded-md bg-light-blue-100 text-light-blue-600 text-sm font-semibold px-4 py-2 dark:text-gray-200 dark:bg-green-700 dark:text-green-100 dark:text-gray-200" href="{{ route('blogs.create') }}">
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @foreach ($blogs as $blog)
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td class="px-4 py-3">
                                    <img src="{{ Storage::url($blog->img_path) }}" style="max-height:50px";>
                                </td>
                                ...
                                <td class="px-4 py-3">          
                                    <div class="flex items-center space-x-4 text-sm">
                                        <a class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-green-600 rounded-lg focus:outline-none focus:shadow-outline-gray" href="{{ route('blogs.show', $blog->id) }}">
                                        ...
                                        <a class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-green-600 rounded-lg focus:outline-none focus:shadow-outline-gray" href="{{ route('blogs.edit', $blog->id) }}">
                                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor"
                                                viewBox="0 0 20 20">
                                                <path
